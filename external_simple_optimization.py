...
class stack_opt_problem(ElementwiseProblem):
    ...
    def set_system(self, filename, x):
        ...
        [tau, Dm_in, Dch_in, Lch, Lch_out, Dch_out, Dm_out] = x
        # set parameters
        ...
        # set flow rate
        data["conditions"]["Q_in"]["value"] = [float(Q_in), float(Q_in)]
        # set pressure 
        ...
        data["manifold"][1]["diameter"]["value"] = float(Dm_in)
        # set outlet channels
        data["channel"][2]["diameter"]["value"] = float(Dch_out)
        data["channel"][2]["length"]["value"] = float(Lch_out)
        data["channel"][3]["diameter"]["value"] = float(Dch_out)
        data["channel"][3]["length"]["value"] = float(Lch_out)
        # set outlet manifolds
        data["manifold"][2]["diameter"]["value"] = float(Dm_out)
        data["manifold"][3]["diameter"]["value"] = float(Dm_out)
        # update json file
