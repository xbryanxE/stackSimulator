...
class stack_opt_problem(ElementwiseProblem):
    def __init__(self, **kwargs):
        ...
        super().__init__(n_var=8, n_obj=2, xl=xl_, xu=xu_, **kwargs)
        # filename
        self.path = "templates/"
    ...
    def set_system(self, filename, x):
        ...
        As = np.pi / 4 * (H**2 - (2*Dm_in**2 + Dm_l**2 - Dm_k**2)) # m^2
        ...
        data["manifold"][2]["diameter"]["value"] = float(Dm_l)
        data["manifold"][3]["diameter"]["value"] = float(Dm_k)
        # update json file
